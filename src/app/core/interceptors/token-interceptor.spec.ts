...
describe('TokenInterceptor', () => {
  ...
  xit('should append token when url does not has http scheme', () => {
    init('', 'token');
  ...
  xit('should append token when url does not has http and base url not empty', () => {
    init(baseUrl, 'token');
  ...
  xit('should append token when url include base url', () => {
    init(baseUrl, 'token');
  ...
  xit('should not append token when url not include baseUrl', () => {
    init(baseUrl, 'token');
  ...
  xit('should not append token when base url is empty and url is not same site', () => {
    init('', 'token');
  ...
  xit('should clear token when response status is unauthorized', () => {
    init('', 'token');
  ...
  xit('should navigate /auth/login when api url is /auth/logout and token is valid', () => {
    init('', 'token');
  ...
  xit('should navigate /auth/login when api url is /auth/logout and token is invalid', () => {
    init('', '');
    const navigateByUrl = spyOn(router, 'navigateByUrl');
    navigateByUrl.and.returnValue(Promise.resolve(true));
